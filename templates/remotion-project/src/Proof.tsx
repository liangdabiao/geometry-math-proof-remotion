...
import {TopBar, TopBarData} from './top-bar';
...
export const Proof: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg, color: C.text}}>
      ...
      <TopBar frame={frame} data={TOPBAR} activeFrom={F.hook} activeTo={F.end}/>
      {/* ⚠️ 多章节必须用条件渲染防止 SVG 透明穿透重叠:
          {frame >= F.renderA - 30 && frame < F.renderB && <RenderA frame={frame}/>}
          {frame >= F.renderB - 30 && frame < F.renderC && <RenderB frame={frame}/>}
          不要同时渲染所有章节 — 见 SKILL.md "常见坑 · SVG 透明穿透" */}
