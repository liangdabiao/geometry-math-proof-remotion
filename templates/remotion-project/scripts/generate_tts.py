...
# MiniMax 凭据位置: 在项目根目录的 .env 文件中查找 `minimaxi=xxx`
# 模板项目自带 .env.example,复制为 .env 后填入真实 API Key 即可。
# 查找顺序: 项目根 .env → 上级 .env → 祖父 .env(向后兼容旧布局)
def _load_api_key() -> str:
    candidates = [ROOT / ".env", ROOT.parent / ".env", ROOT.parent.parent / ".env"]
    for env_path in candidates:
        if not env_path.exists():
            continue
        m = re.search(r"minimaxi\s*=\s*(\S+)", env_path.read_text(encoding="utf-8"))
        if m and m.group(1) and m.group(1) != "your_api_key_here":
            return m.group(1)
    raise RuntimeError(
        "未找到 MiniMax API Key。请在项目根 .env 中设置 minimaxi=<your-key>。\n"
        "可参考 .env.example。"
    )
