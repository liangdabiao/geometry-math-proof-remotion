...
## 10. ⚠️ SVG 透明穿透（最常见 Bug）

**症状**：多章节视频中前章节图形透过当前章节空白区域显示，所有内容叠在一起。

**根因**：SVG `<svg>` 背景全透明。各章节用 `fadeIn(f, F.xxx, 30)` 从 0 淡入到 1，但从不淡出。前章节图形透过当前章节 SVG 透明区域持续可见。

**必须用条件渲染**：React mount/unmount 替代 opacity 控制：

```tsx
// ❌ 错：所有章节同时渲染，透明穿透
<RenderChapterA frame={frame}/>
<RenderChapterB frame={frame}/>

// ✅ 对：每章只在自己的帧范围挂载
{frame >= F.circuit - 30 && frame < F.switch && <RenderChapterA frame={frame}/>}
{frame >= F.switch - 30 && frame < F.fuse && <RenderChapterB frame={frame}/>}
```

**边界规则**：起始 `F.当前章 - 30`（提前 30 帧让 fadeIn 完成），截止 `F.下章`（硬边界 unmount）。

**不适用**：全局组件（BackgroundGrid、FormulaPanel、CaptionStrip）不逐章替换，不受影响。
